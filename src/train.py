...
if train_type == "one-hot":
    ...
    if argparser.parse_args().cond:
        data_config = load_config(
            "/home/admin/workspace/yuyuanhong/code/CityLayout/config/data/osm_cond_loader_rgb.yaml"
        )
        trainer_config = load_config(
            "/home/admin/workspace/yuyuanhong/code/CityLayout/config/train/osm_cond_generator_rgb.yaml"
        )
    else:
        data_config = load_config(
            "/home/admin/workspace/yuyuanhong/code/CityLayout/config/data/osm_loader_rgb.yaml"
        )
        trainer_config = load_config(
            "/home/admin/workspace/yuyuanhong/code/CityLayout/config/train/osm_generator_rgb.yaml"
        )
...
if argparser.parse_args().eval:
    if train_type == "one-hot":
        if argparser.parse_args().cond:
            ...
            trainer_config = load_config(
                "/home/admin/workspace/yuyuanhong/code/CityLayout/config/train/osm_cond_generator_sample.yaml"
            )
        else:
            data_config = load_config(
                "/home/admin/workspace/yuyuanhong/code/CityLayout/config/data/osm_loader.yaml"
        ...
        if argparser.parse_args().cond:
            ...
            trainer_config = load_config(
                "/home/admin/workspace/yuyuanhong/code/CityLayout/config/train/osm_cond_generator_rgb_sample.yaml"
            )
        else:
            data_config = load_config(
                "/home/admin/workspace/yuyuanhong/code/CityLayout/config/data/osm_loader_rgb.yaml"
...
model = Unet(
    ...
    init_dim=None
    if trainer_config["model"]["init_dim"] == "None"
    else trainer_config["model"]["init_dim"],
    out_dim=None
    if trainer_config["model"]["out_dim"] == "None"
    else trainer_config["model"]["out_dim"],
    ...
    full_attn=None
    if trainer_config["model"]["full_attn"] == "None"
    else trainer_config["model"]["full_attn"],
...
diffusion = GaussianDiffusion(
    ...
    sampling_timesteps=trainer_config["diffusion"][
        "ddim_timestep"
...
if trainer_config["trainer"]["mode"] == "eval":
    vis = OSMVisulizer(
        trainer_config["vis"]["channel_to_rgb"],
        threshold=trainer_config["vis"]["threshold"],
        path=trainer_config["vis"]["path"],
    )
    vec = Vectorizer(config=trainer_config["vec"])

    ret, ref = trainer.sample(
        trainer_config["trainer"]["num_samples"],
        trainer_config["trainer"]["batch_size"],
        trainer_config["trainer"]["milestone"],
        trainer_config["trainer"]["condition"],
    )
